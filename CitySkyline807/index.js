const maxIncreaseKeepingSkyline = function(A) {
    let res =[]
    let count= 0
    for (let i = 0; i < A.length; i++) {
        ...
        for (let j = 0; j < B.length; j++) {
            ...
            for (let k = 0; k < A.length; k++) {
                columnArray.push(A[k][j])
            }

            let maxValRow = Math.max(...B)
            let maxValColumn = Math.max(...columnArray)
            let compareVal = Math.min(maxValRow, maxValColumn)
            if(compareVal>B[j]){
                res[i][j] = compareVal
                count += compareVal - B[j]
            }else {
                res[i][j] = B[j]
            }
        }
 ...
 */
const maxIncreaseKeepingSkyline1 = function(grid) {
...
const findMaxRowsCols = function(grid, rowM, colsM){
